...
public class DiagonalShapes {
	...
	public Shape cutter(int length, Shape[] shapes, Shape[] opponent_shapes)
	{
		...
		if (row_2.length != cutter.length - 1) {
			...
			if(length==11){
				return ShapesBasket.generateDiag11( );
				
			}else if(length==8){
				return new Shape(ShapesBasket.get8Shape_ThickI());
				
			}else if(length==5){
				 cutter=ShapesBasket.get5Shape_I();//get5Shape_ZigZag();
			...
			if(length==11 && shapeIndex[2] <1){
				...
				if(shapeIndex[1]==0){
					return ShapesBasket.generateDiag8( );
				}
